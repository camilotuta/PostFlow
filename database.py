...
COL_TZ = pytz.timezone(config.TIMEZONE)
...
class Video(db.Model):
    ...
    brand = db.Column(
        db.String(32), nullable=False, default="gymark"
    )  # gymark | tatuct
    file_path = db.Column(db.String(512), nullable=False)
    source_file_path = db.Column(db.String(512), nullable=True)
    ...
    posts = db.relationship(
        "Post", backref="video", lazy=True, cascade="all, delete-orphan"
    )
    ...
    def _path_to_url(path: str | None) -> str | None:
        ...
        if not txt:
            return None
        try:
            rel = os.path.relpath(txt, config.BASE_DIR).replace("\\", "/")
        except Exception:
            return None
        if rel.startswith(".."):
            return None
    ...
    def get_source_path(self) -> str:
        return str(self.source_file_path or self.file_path or "")

    def get_publish_path(self) -> str:
        return str(self.processed_file_path or self.file_path or "")
    ...
    def to_dict(self):
        ...
        return {
            ...
            "uploaded_at": (
                self.uploaded_at.strftime("%Y-%m-%d %H:%M") if self.uploaded_at else ""
            ),
            "category_id": self.category_id,
            "ai_title": self.ai_title,
            "ai_description": self.ai_description,
            "video_url": self._path_to_url(active_path),
            "source_video_url": self._path_to_url(self.get_source_path()),
            "is_customized": bool(self.processed_file_path),
...
class Post(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            "posted_at": (
                self.posted_at.strftime("%Y-%m-%d %H:%M") if self.posted_at else None
            ),
            "status": self.status,
